/**
 * Subtraction Practice Session Manager
 * Manages practice sessions, tracks results, updates mastery with inverse linking.
 */

import { generateSubtractionProblem, getFactsByStrategy, getAllSubtractionFacts } from './subtraction-facts.js';
import { updateFactMastery } from '../data/expansion-storage.js';
import { shuffle, getInverseFacts } from '../expansion-utils.js';
...
function generateProblems(strategyId, count) {
  if (strategyId === 'mixed') {
    ...
    return shuffled.slice(0, count).map(fact => ({
      ...generateSubtractionProblem(fact.strategy),
      a: fact.a,
      b: fact.b,
      answer: fact.answer
    }));
    ...
    for (let i = 0; i < count; i++) {
      problems.push(generateSubtractionProblem(strategyId));
    }
    return problems;
  }
}
 ...
 * Updates both subtraction fact and linked addition facts.
...
export function submitAnswer(answer) {
  ...
  // Update mastery for this subtraction fact
  updateFactMastery(problem.factKey, correct, timeTaken);

  // Update inverse addition facts mastery
  // For a - b = c, the inverse facts are b + c = a and c + b = a
  try {
    const inverseFacts = getInverseFacts(problem.factKey);
    inverseFacts.forEach(inverseKey => {
      // Update inverse facts with same result (correct/incorrect)
      // but don't penalize time as heavily since they weren't directly practiced
      updateFactMastery(inverseKey, correct, timeTaken * 1.5);
    });
  } catch (error) {
    // Silently fail if inverse facts can't be computed
    console.warn('Could not update inverse facts:', error);
  }
